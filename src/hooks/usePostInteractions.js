...
import { createAndPublishEvent } from '../utils/nostrClient';
...
export const usePostInteractions = ({
  ...
  const handleLike = useCallback(async (post) => {
    try {
      ...
      // Use nostr-tools to publish
      await createAndPublishEvent(likeEvent);
  ...
  const handleRepost = useCallback(async (post) => {
    try {
      ...
      // Use nostr-tools to publish
      await createAndPublishEvent(repostEvent);
  ...
  const handleZap = useCallback(async (post, wallet) => {
    ...
    try {
      ...
      // Use nostr-tools to publish
      await createAndPublishEvent(zapEvent);
  ...
  const handleComment = useCallback(async (postId) => {
    ...
    try {
      ...
      // Use nostr-tools to publish
      await createAndPublishEvent(commentEvent);
            ...
            return {
              ...
              comments: [
                ...post.comments,
                {
                  ...
                  author: {
                    pubkey: await getPublicKey(), // Get the user's public key
                    profile: userProfile
                  }
                }
              ]
