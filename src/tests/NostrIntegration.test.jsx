...
describe('Nostr Integration Tests', () => {
  ...
  it('should save run data according to NIP-101e standard', async () => {
    // Render the RunTracker component
    const { unmount } = renderWithRouter(<RunTrackerComponent />);
    ...
    // Now go to run history and simulate saving to nostr
    renderWithRouter(<RunHistory />, { route: '/history' });
    
    // Simulate saving to nostr
    await act(async () => {
      // Directly call the nostr publication functions
      await createAndPublishEvent({
        kind: 1301,
        ...
        tags: [
          ["workout", savedRun.name || "Untitled Run"],
          ["exercise", "running"],
          ["distance", "5.00", "km"],
          ["duration", "30:00"],
          ["elevation_gain", "50", "m"]
    ...
    // Verify that createAndPublishEvent was called with the correct NIP-101e format
    expect(createAndPublishEvent).toHaveBeenCalled();
    const calledArg = createAndPublishEvent.mock.calls[0][0];
    
    expect(calledArg.kind).toBe(1301);
    expect(calledArg.content).toBe("Completed a run with RUNSTR!");
    expect(calledArg.tags).toContainEqual(["workout", savedRun.name || "Untitled Run"]);
    expect(calledArg.tags).toContainEqual(["exercise", "running"]);
    expect(calledArg.tags).toContainEqual(["distance", "5.00", "km"]);
    expect(calledArg.tags).toContainEqual(["duration", "30:00"]);
    expect(calledArg.tags).toContainEqual(["elevation_gain", "50", "m"]);
  });
}); 