...
describe('RunHistory Component', () => {
  ...
  it('should handle run deletion', async () => {
    ...
    // Check that the run was marked as deleted in localStorage
    const updatedHistory = JSON.parse(localStorage.getItem('runHistory'));
    const deletedRun = updatedHistory.find(run => run.id === mockRuns[0].id);
    expect(deletedRun.deleted).toBe(true);
  });

  it('should recalculate stats after run deletion', async () => {
    renderWithRouter(<RunHistory />);
    ...
    // Get initial stats
    const initialStats = runDataService.calculateStats();
    ...
    await waitFor(() => {
      const updatedStats = runDataService.calculateStats();
      expect(updatedStats.totalRuns).toBe(initialStats.totalRuns - 1);
      expect(updatedStats.totalDistance).toBeLessThan(initialStats.totalDistance);
      expect(updatedStats.totalDuration).toBeLessThan(initialStats.totalDuration);
    });
  });
  ...
  it('should handle deletion errors gracefully', async () => {
    ...
    // Mock runDataService.deleteRun to throw an error
    const originalDeleteRun = runDataService.deleteRun;
    runDataService.deleteRun = vi.fn().mockRejectedValue(new Error('Deletion failed'));
    ...
    // Restore original deleteRun function
    runDataService.deleteRun = originalDeleteRun;
  });
