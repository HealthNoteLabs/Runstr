...
/**
 * Formats run data according to NIP-101e standard
 * @param {Object} runData - The run data to format
 * @param {string} runData.name - The name of the run
 * @param {number} runData.distance - Distance in meters
 * @param {number} runData.duration - Duration in seconds
 * @param {Object} runData.elevation - Elevation data
 * @param {number} runData.elevation.gain - Elevation gain in meters
 * @param {number} runData.elevation.loss - Elevation loss in meters
 * @param {string} distanceUnit - The unit to use for distance ('km' or 'mi')
...
/**
 * Formats health profile event according to NIP-30001 standard
 * @param {Object} profileData - The health profile data
 * @param {number} profileData.weight - User's weight
 * @param {number} profileData.height - User's height
 * @param {string} profileData.gender - User's gender
 * @param {string} weightUnit - The unit to use for weight ('kg' or 'lbs')
 * @param {string} heightUnit - The unit to use for height ('cm' or 'in')
 * @returns {Object} Formatted NIP-30001 event
 */
export const formatHealthProfileEvent = (profileData, weightUnit = 'kg', heightUnit = 'cm') => {
  return {
    kind: 30001,
    content: "RUNSTR health profile",
    tags: [
      ["d", "health-profile"],
      ["weight", profileData.weight.toString(), weightUnit],
      ["height", profileData.height.toString(), heightUnit],
...
/**
 * Formats health record event according to NIP-30002 standard
 * @param {Object} healthData - The health data
 * @param {number} healthData.weight - Current weight
 * @param {number} healthData.restingCalories - Calculated resting calories
 * @param {string} workoutId - ID of related workout event
 * @param {string} weightUnit - The unit to use for weight ('kg' or 'lbs')
 * @returns {Object} Formatted NIP-30002 event
 */
export const formatHealthRecordEvent = (healthData, workoutId, weightUnit = 'kg') => {
  const timestamp = Math.floor(Date.now() / 1000);
  const date = new Date().toISOString().split('T')[0];
  
  return {
    kind: 30002,
    content: "Health data from RUNSTR",
    tags: [
      ["d", `health-${date}`],
      ["timestamp", timestamp.toString()],
      ["weight", healthData.weight.toString(), weightUnit],
      ["resting_calories", healthData.restingCalories.toString(), "kcal"],
...
/**
 * Formats enhanced workout record with health data
 * @param {Object} runData - The run data to format
 * @param {number} caloriesBurned - Calories burned during the workout
 * @param {string} distanceUnit - The unit to use for distance ('km' or 'mi')
 * @returns {Object} Formatted NIP-101e event with health data
 */
