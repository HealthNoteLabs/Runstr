...
// Additional relays for other features
const ADDITIONAL_RELAYS = [
  'wss://purplepag.es'
];

// Combined relay list for backward compatibility
const relays = [...FEED_RELAYS, ...ADDITIONAL_RELAYS, ...GROUP_RELAYS];
...
export const initializeNostr = async () => {
  try {
    ...
    // Connect to remaining relays
    for (const relay of [...GROUP_RELAYS.slice(1), ...ADDITIONAL_RELAYS]) {
      try {
        const conn = await pool.ensureRelay(relay);
        if (conn) {
          connectedRelays.push(relay);
